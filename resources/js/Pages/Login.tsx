...
import {
  ...
  User, Sun, Moon, ArrowRight, ArrowLeft, CheckCircle2, ShieldCheck, Layers, RefreshCw,
  Send, ShieldAlert
} from "lucide-react";
...
export default function LoginPage({ initialView }: Props) {
  const { t } = useTranslation();
  const { theme, setTheme } = useTheme();
  const login = useAuth((s) => s.login);

  // Card view state: 'signin' | 'forgot'
  const [view, setView] = useState<"signin" | "forgot">(() => {
    if (typeof window !== "undefined") {
      const p = new URLSearchParams(window.location.search);
      if (p.get("view") === "forgot-password") return "forgot";
    }
    return initialView === "forgot-password" ? "forgot" : "signin";
  });
  ...
  const switchView = (targetView: "signin" | "forgot") => {
    if (targetView === view) return;
    setSlideDirection(targetView === "forgot" ? 1 : -1);
    setView(targetView);
    ...
    if (targetView === "forgot") {
      ...
      window.history.replaceState(null, "", "?view=forgot-password");
    } else {
  ...
  const handleForgotSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setForgotSent(true);
      toast.success(res.data?.message || "Reset link sent to your Telegram account!");
    } catch (error: any) {
      const msg =
        error.response?.data?.message ||
        error.response?.data?.errors?.email?.[0] ||
        "Failed to send reset link.";
      setUnlinkedError(msg);
      toast.error(msg);
    } finally {
  ...
  return (
    <div className="relative flex min-h-screen items-center justify-center bg-background px-4 py-12 transition-colors duration-300">
      ...
      <div className="relative z-10 grid w-full max-w-5xl gap-8 md:grid-cols-[1.1fr_1fr]">
        ...
        {/* AUTH FORM CARD (Mobile & Desktop) */}
        <div className="rounded-3xl border border-border/80 bg-card p-8 shadow-2xl backdrop-blur-2xl sm:p-10 flex flex-col justify-center relative overflow-hidden min-h-[580px]">
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button
                    ...
                    className="text-xs font-semibold text-muted-foreground hover:text-foreground flex items-center gap-1 transition-colors px-2 py-1 rounded-lg hover:bg-muted"
                ...
                <p className="text-xs text-muted-foreground mb-6">
                  {forgotSent
                    ? "Check your Telegram for your 5-minute single-use reset link."
                    : "Enter your account email to receive your password reset link via Telegram."}
                </p>

                {unlinkedError && (
                  <div className="mb-4 rounded-2xl border border-destructive/30 bg-destructive/10 p-3.5 text-xs text-destructive flex items-start gap-2.5">
                    <ShieldAlert className="h-4 w-4 shrink-0 mt-0.5" />
                    <div>
                      <span className="font-bold block mb-0.5">Account Not Linked</span>
                      <span className="text-[11px] leading-relaxed">{unlinkedError}</span>
                    </div>
                  </div>
                )}
                ...
                {!forgotSent ? (
                  <form onSubmit={handleForgotSubmit} className="space-y-4">
                    ...
                    <Button
                      type="submit"
                      className="w-full h-11 text-sm font-bold rounded-xl bg-primary text-primary-foreground shadow-lg shadow-primary/25 hover:bg-primary/90 transition-all mt-2 gap-2"
                      disabled={forgotLoading}
                    >
                      <Send className="h-4 w-4" />
                      {forgotLoading ? "Sending to Telegram..." : "Send Reset Link to Telegram"}
                    </Button>
                  ...
                  <div className="space-y-5">
                    <div className="rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-5 text-center">
                      ...
                      <p className="text-xs text-muted-foreground mt-1.5 leading-relaxed">
                        A single-use reset link has been dispatched to your linked Telegram account. It is valid for <strong>5 minutes</strong>.
                      </p>
                    </div>
                    ...
                    <div className="flex flex-col gap-2">
                      <Button
                        type="button"
                        onClick={() => switchView("signin")}
                        className="w-full h-11 text-sm font-bold rounded-xl bg-primary text-primary-foreground shadow-md"
                      >
                        Back to Sign In
                      </Button>
                      <button
                        type="button"
                        onClick={() => { setForgotSent(false); setUnlinkedError(null); }}
                        className="text-xs text-muted-foreground hover:text-foreground underline mt-1 text-center"
                      >
                ...
                <p className="mt-8 text-center text-[11px] text-muted-foreground">
                  Accounts are provisioned internally by your administrator.
                </p>
              </motion.div>
            )}
          </AnimatePresence>
